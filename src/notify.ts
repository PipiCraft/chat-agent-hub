...
export async function sendNotification(message: string, agentName: string = "智能体", targetPathOrName: string = process.cwd()): Promise<boolean> {
    ...
    if (shouldNotifyChannel(config, "feishu") && config.channels?.feishu?.appId && config.channels?.feishu?.appSecret) {
        try {
            ...
            let feishuTargetId: string | null = null;
